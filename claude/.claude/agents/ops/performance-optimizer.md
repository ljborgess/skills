...
description: Agente de velocidade e eficiência. Use quando o usuário diz "está lento", "como otimizar", "a página demora", "tem N+1", ou descreve uma regressão de performance. Especialista em frontend (Core Web Vitals, bundle), backend (queries, cache, async) e banco de dados — mede primeiro, nunca otimiza no escuro.
