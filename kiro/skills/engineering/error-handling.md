...
class AppError extends Error {
  constructor(
    ...
    public readonly isOperational: boolean = true, // erro esperado vs bug
...
export function errorHandler(
  ...
  // Nunca exponha detalhes de bugs ao usuário
  res.status(500).json({
    error: {
      code: 'INTERNAL_ERROR',
...
export const logger = pino({
  ...
  // Em produção: JSON puro para ingestão por Datadog/CloudWatch/etc
  // Em dev: pretty print
...
async function withRetry<T>(
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    try {
      ...
      const jitter = Math.random() * delay * 0.1 // evita thundering herd
